...
describe('XLSX 分镜导出', () => {
  it('输出录制小节、每个子镜头一行，并嵌入支持的图片', async () => {
    ...
    expect(sheet.getRow(1).values).toEqual([, '分镜序号', '录制小节', '乐器/声部', '景别', '描述', '示意图']);
    expect(sheet.getRow(2).values).toEqual([, 1, 'M.1-2（2 小节）', '小提琴', '近景', '左手换把', '']);
